...
class Output(Agent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.option("name", "str", "Output name")
    ...
    def setup(self):
        ...
        with open(f"/sys/class/gpio/gpio{name}/value", "wt") as self.fd:
            yield
            self.fd = None
...
class Input(Agent):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.option("name", "str", "Input name")
    ...
    def setup(self):
        ...
        # Open value file.
        with open(f"/sys/class/gpio/gpio{name}/value", "rt") as self.fd:
            # Dispatch a dedicated thread for selecting the value file.
            threading.Thread(target=self.select, name=f"GPIO {name} select",
                             daemon=True)
            # Yield
            yield
            self.fd = None
